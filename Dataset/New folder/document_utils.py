...
# ==========================================================
# SAVE DOCUMENT IN ANY TARGET FORMAT (docx / pdf / txt)
# ==========================================================
# Builds the document exactly as before (python-docx), then
# converts it to the requested format using LibreOffice in
# headless mode. This keeps every add_* function above
# completely unchanged — only the final save step differs.
#
# Requires LibreOffice installed and the `soffice` command
# available on PATH (free, standard on Linux; installable on
# Windows/Mac from libreoffice.org).

# Maps our format names to the LibreOffice filter name it expects
...
def save_document_as(doc, filepath, target_format):
    ...
    if target_format == "xlsx":
        _save_docx_as_xlsx(doc, filepath)
        return

    if target_format not in _LIBREOFFICE_FILTERS:
        raise ValueError(
            f"Unsupported target_format '{target_format}'. "
            f"Use 'docx', 'pdf', 'txt', or 'xlsx'."
        )

    # Save a temporary .docx next to the target file, convert it,
    # then remove the temporary file.
    temp_docx = os.path.splitext(filepath)[0] + "__temp.docx"
    doc.save(temp_docx)

    out_dir = os.path.dirname(filepath) or "."
    filter_name = _LIBREOFFICE_FILTERS[target_format]

    result = subprocess.run(
        [
            "soffice", "--headless", "--convert-to", filter_name,
            "--outdir", out_dir, temp_docx
        ],
    ...
    if result.returncode != 0:
        os.remove(temp_docx)
        raise RuntimeError(
            f"LibreOffice conversion failed for {filepath}: {result.stderr}"
        )

    # LibreOffice names the output after the temp file — rename it
    # to the real target filename.
    converted = os.path.splitext(temp_docx)[0] + f".{target_format}"
    os.replace(converted, filepath)
    os.remove(temp_docx)


def _iter_block_items(document):
    """
    Walks a python-docx Document's body in the actual order things
    were added (headings, paragraphs, tables interleaved) — normal
    python-docx only gives you doc.paragraphs and doc.tables as two
    separate lists, losing the order between them.
    """
...
def _save_docx_as_xlsx(doc, filepath):
    """
    Converts the already-built docx Document object into a
    spreadsheet: headings become bold section rows, paragraph text
    becomes plain rows, and each docx table becomes a block of
    rows/columns — in the same order they were added. No changes
    needed to any add_* function or the generator scripts.
    """
    wb = Workbook()
    ws = wb.active
    ws.title = "Document"
    ...
    row = 1
    bold = Font(bold=True)
    heading_font = Font(bold=True, size=13)
    ...
    for block in _iter_block_items(doc):
        ...
        if isinstance(block, Paragraph):
            ...
            is_heading = block.style.name.startswith("Heading") or block.style.name == "Title"

            cell = ws.cell(row=row, column=1, value=text)
            cell.font = heading_font if is_heading else Font(bold=False)

            row += 1

        elif isinstance(block, Table):
            for table_row in block.rows:
                for col_idx, cell in enumerate(table_row.cells, start=1):
                    out_cell = ws.cell(row=row, column=col_idx, value=cell.text.strip())
                    if col_idx == 1:
                        out_cell.font = bold
                row += 1
...
def add_workorder_information(doc, event):
    ...
    values = [
        ...
        ("Work Order Number", f"WO_{event['Event ID']}"),
...
def add_incident_information(doc, event):
    ...
    values = [
        ...
        ("Incident Number", f"INC_{event['Event ID']}"),
